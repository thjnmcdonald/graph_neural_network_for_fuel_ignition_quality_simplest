...
class Net(torch.nn.Module):
    def __init__(self):
        ...
        if model_type_name == 'GCN_single':
            self.convGNN = GCNConv(dataset.num_features, 32, bias = False)
            
        elif model_type_name == 'GCN_multiple':
            self.convGNN1 = GCNConv(dataset.num_features, 64, bias = False)
            self.convGNN2 = GCNConv(64, 32, bias = False)
            self.convGNN3 = GCNConv(32, 32, bias = False)
        
        elif model_type_name == 'graphconv_single':
            self.graphconv1 = GraphConv(dataset.num_features, 32, bias = False)
        
        elif model_type_name == 'graphconv_multiple':
            self.graphconv1 = GraphConv(dataset.num_features, 32, bias = False)
            self.graphconv2 = GraphConv(32, 64, bias = False)
            self.graphconv3 = GraphConv(64, 32, bias = False)
    ...
    def forward(self, data, print_intermediaries = False, model_type = model_type_name):
        ...
        if model_type == 'graphconv_single':
            out_GNN = F.relu(self.graphconv1(data.x, data.edge_index))
        
        elif model_type == 'graphconv_multiple':
            GCN_1 = F.relu(self.graphconv1(data.x, data.edge_index))
            GCN_2 = F.relu(self.graphconv2(GCN_1, data.edge_index))
            out_GNN = F.relu(self.graphconv3(GCN_2, data.edge_index))
        
        elif model_type == 'GCN_single':
            out_GNN = F.relu(self.convGNN(data.x, data.edge_index))
        
        elif model_type == 'GCN_multiple':
            out_GCN_1 = F.relu(self.convGNN1(data.x, data.edge_index))
            out_GCN_2 = F.relu(self.convGNN2(out_GCN_1, data.edge_index))
            out_GNN = F.relu(self.convGNN3(out_GCN_2, data.edge_index))
        ...
        out_scatter = scatter_add(out_GNN, data.batch, dim=0)
        ...
        for data in loader:
            ...
            for mol in mol_id:
                ...
                if tmp_mol_name == 'CCCCl':
                    ...
                    intermediate_values = model(data, print_intermediaries = True)
...
def preprocess_save(dataset_folder_name):
    ...
    save_path = save_dir  + train_id + '-' +  model_type_name
...
torch.save(state_dict, save_path + "/simplest_model.tar")
