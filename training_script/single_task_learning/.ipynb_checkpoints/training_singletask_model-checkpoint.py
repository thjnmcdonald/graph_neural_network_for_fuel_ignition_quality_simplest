...
parser.add_argument('--target', default='TB')   # define target property for single task learning (DCN, MON, or RON)
...
class Net3(torch.nn.Module):
    def __init__(self):
        ...
        # convert the 12 edge features to in channels, out channels of 
        nn = Sequential(Linear(12, 128), ReLU(), Linear(128, dim * dim))

        # self.conv = NNConv(dim, dim, nn, aggr='add')
        # self.gru = GRU(dim, dim)
        
        self.convGNN = GCNConv(dataset.num_features, dim)
        
        # GraphConv komt uit kgnn
        # self.lin2 = torch.nn.Linear(dim + num_i_2, dim)
        # self.conv4 = GraphConv(dim, dim)
        # self.gru2 = GRU(dim, dim)
    ...
    def forward(self, data):
        # out = F.relu(self.lin0(data.x)) # node feature matrix
        
        for i in range(conv_type):
            out = F.relu(self.convGNN(data.x, data.edge_index))
        ...
        x_forward = out
        # print("b4: " + str(x_forward.size()))
        
        x_1 = scatter_add(x_forward, data.batch, dim=0)
        # print("after: " + str(x_1.size()))
...
#         # dit heeft te maken met die higher order dingen. 
#         for i in range(conv_type2):
#             m = F.relu(self.conv4(out, data.edge_index_2))
#             out, h = self.gru2(m.unsqueeze(0), h)
#             out = out.squeeze(0)
