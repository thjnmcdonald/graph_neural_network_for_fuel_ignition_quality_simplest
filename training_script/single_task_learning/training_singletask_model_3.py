...
class Net(torch.nn.Module):
    ...
    def forward(self, data, print_intermediaries = False):
        ...
        out_GCN = F.relu(self.convGNN(data.x, data.edge_index))
        # print(f'>this is what we are looking for?: {self.training}')

        out_scatter = scatter_add(out_GCN, data.batch, dim=0)
        ...
        if print_intermediaries == True:
            intermediate_values = [data.x, data.edge_index, out_GCN, out_scatter, out_ANN1, out_final]
