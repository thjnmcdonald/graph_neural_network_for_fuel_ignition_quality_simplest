...
target_name = 'DCN_MON_RON_joined'   # only multitask model for DCN, MON, RON is implemented yet
...
parser.add_argument('--top_model_dataset', default='DCN_MON_RON_joined/Default/')   # dataset directory for retraining model
...
dataset_folder_name = top_model_dataset   # no base_model with multitask data available (only DCN data available for transfer learning)
...
mean_lst = []
std_lst = []
## Make sure both datasets are standardized with same mean and standard deviation for target properties that are used in pretraining (here mean and std of IQT-DCN training data of Yanowitz Compendium is used for pretraining standardization)
#mean == [32.99351119995117]   # mean corresponds to mean of DCN training data of Yanowitz Compendium
#std == [23.430192947387695]   # std corresponds to std of DCN training data of Yanowitz Compendium
